...
def update(request, day_offset, no_to_display=default_length):
    ...
    try:
        ...
        articles = Article.objects.filter(
            pub_date__gte=day_before, pub_date__lte=date_request
        )
        print(
            "Showing papers from "
            + datetime.datetime.strftime(day_before, "%Y-%m-%d")
            + ". No. of papers: "
            + str(len(articles))
        )
        ...
        if day_offset == 0:
            context = {
                "dateToDisplay": date_request,
                "go_back_url": reverse("update", args=(day_offset + 1, default_length)),
                "go_forward_url": reverse("update", args=(day_offset, default_length)),
                "current_url": reverse("update", args=(day_offset, 7)),
                "highest_ranked_list": highest_ranked_list,
                "whitelist": whitelist,
                "blacklist": blacklist,
            }
        else:
            context = {
                "dateToDisplay": date_request,
                "go_back_url": reverse("update", args=(day_offset + 1, default_length)),
                "go_forward_url": reverse(
                    "update", args=(day_offset - 1, default_length)
                ),
                "current_url": reverse("update", args=(day_offset, 7)),
                "highest_ranked_list": highest_ranked_list,
                "whitelist": whitelist,
